...
Provenance: user-supplied submission_23.tar.gz -- a live-decision
heuristic engine (job-priority queue + greedy worker assignment), distinct
from the tape-replay family agent/main.py and
baselines/frontier_router_original.py belong to. Includes a tuned "Claude
knob layer" (see PK dict below) overriding RESERVE_FRACTION multiplier,
herd sizing, melon tile targets, glide-out timing, etc.

Kept here only as an A/B opponent for tools/run_ab.py -- see
experiments/2026-08-04_v24_vs_submission23/RESULTS.md for the real-engine
result (agent/main.py beat this file 20/20, mean margin +18,185) and
RECOMMENDATIONS.md for a latent bug found in the PK knob layer
(FEED_STOCK_DAYS is tuned then immediately overwritten, so that knob never
takes effect).
"""
